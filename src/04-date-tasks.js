...
function angleBetweenClockHands(date) {
  const angleM = (360 / 60) * date.getUTCMinutes();
  const angleH = (360 / 12) * (date.getUTCHours() % 12) + (360 / 12 / 60) * date.getUTCMinutes();

  let angle = Math.max(angleM, angleH) - Math.min(angleM, angleH);
  if (angle > 180) {
    angle -= 180;
  }
